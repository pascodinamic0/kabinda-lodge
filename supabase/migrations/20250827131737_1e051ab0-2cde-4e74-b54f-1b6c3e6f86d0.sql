...
-- Create audit trigger for payment data access
CREATE OR REPLACE FUNCTION public.audit_payment_access()
RETURNS TRIGGER AS $$
BEGIN
  -- Log all payment data access attempts
  PERFORM log_sensitive_data_access('payments', 'SELECT', NEW.id::text);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Add trigger to log payment access
DROP TRIGGER IF EXISTS trigger_audit_payment_access ON public.payments;
CREATE TRIGGER trigger_audit_payment_access
