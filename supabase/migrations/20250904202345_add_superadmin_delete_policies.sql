-- Add DELETE policies for SuperAdmins on bookings and conference_bookings
CREATE POLICY IF NOT EXISTS "SuperAdmins can delete all bookings"
ON public.bookings
FOR DELETE
USING (get_current_user_role() = 'SuperAdmin'::app_role);

CREATE POLICY IF NOT EXISTS "SuperAdmins can delete all conference bookings"
ON public.conference_bookings
FOR DELETE
USING (get_current_user_role() = 'SuperAdmin'::app_role);

-- Also add UPDATE policies for room status updates
CREATE POLICY IF NOT EXISTS "SuperAdmins can update rooms"
ON public.rooms
FOR UPDATE
USING (get_current_user_role() = 'SuperAdmin'::app_role);

CREATE POLICY IF NOT EXISTS "SuperAdmins can update conference rooms"
