-- Add explicit deny-all RLS policies for auth_rate_limit to satisfy linter and block direct access
CREATE POLICY IF NOT EXISTS "No direct select" ON public.auth_rate_limit
FOR SELECT USING (false);

CREATE POLICY IF NOT EXISTS "No direct insert" ON public.auth_rate_limit
FOR INSERT WITH CHECK (false);

CREATE POLICY IF NOT EXISTS "No direct update" ON public.auth_rate_limit
FOR UPDATE USING (false);

CREATE POLICY IF NOT EXISTS "No direct delete" ON public.auth_rate_limit
FOR DELETE USING (false);

-- Ensure key functions have a fixed search_path
