...
export default function SuperAdminDashboard() {
  ...
  const { t } = useLanguage();
  const { totalUsers, totalAdmins, totalRevenue, systemTables, loading, error } = useSuperAdminStats();
          ...
          <Card className="border-amber-200 bg-amber-50 dark:bg-amber-950 dark:border-amber-800">
            <CardHeader>
              <CardTitle className="text-amber-800 dark:text-amber-200 flex items-center gap-2">
                ...
                {t('super_admin_privileges', 'Super Admin Privileges')}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-amber-700 dark:text-amber-300 text-sm">
                {t('super_admin_warning', 'You have access to system-critical functions. Use these privileges responsibly and ensure proper security protocols are followed.')}
              </p>
            </CardContent>
          </Card>
        </div>
