...
async function GET(request) {
    try {
        ...
        try {
            let query = serverSupabase.from('agents').select('*').eq('hotel_id', hotelId).order('last_seen', {
                ascending: false
            });
            if (status) {
            ...
            const agentsWithQueue = await Promise.all((agents || []).map(async (agent)=>{
                try {
                    const { count } = await serverSupabase.from('card_issues').select('*', {
                        count: 'exact',
                        head: true
                    }).eq('agent_id', agent.id).in('status', [
                        'pending',
                        'queued'
                    ]);
                    return {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__4c89366a._.js.map